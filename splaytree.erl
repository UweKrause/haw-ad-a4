...
findTP(_B={W, L, R, _H}, E) when E < W ->
    ...
    % Die Hoehe des gefundenen Knotens wird vom Nachfolger weiter nach oben gereicht
    {SH, L_neu} = findTP(L, E),    
...
rotiere(B, [l,l | _R]) -> zig_zig(B, r);

rotiere(B, [l,r | _R]) -> zig_zag(B, r);

rotiere(B, [r,l | _R]) -> zig_zag(B, l);

rotiere(B, [r,r | _R]) -> zig_zig(B, l).
...
zig_zig(B, Richtung) ->
    %io:format("Zig-Zig ~w~n", [Richtung]),
...
zig_zag(_B={W, L, R, _H}, Richtung) ->
    ...
    %io:format("Zig-Zag ~w~n", [Richtung]),
